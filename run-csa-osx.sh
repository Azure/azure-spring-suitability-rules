...
if [ $# -eq 0 -o $# -gt 2 ]; then 
  echo "[ERROR] No arguments or too many arguments given. Usage: ./run-csa-osx.sh <path> [-p] or ./run-csa-osx.sh [-p] <path>"
  exit 1
fi

if [ $# == 1 ]; then
  if [ $1 == "-p" ]; then
    echo "[ERROR] Invalid argument. Usage: ./run-csa-osx.sh <path> [-p] or ./run-csa-osx.sh [-p] <path>"
    exit 1
  else
    if [ ! -e "$1" ]; then
      echo "[ERROR] Path $1 does not exist. Usage: ./run-csa-osx.sh <path> [-p] or ./run-csa-osx.sh [-p] <path>"
      exit 1
    fi
  fi
fi
...
if [ $# == 2 ]; then 
  if [ $1 != "-p" ] && [ $2 != "-p" ]; then
    echo "[ERROR] Invalid arguments. Usage: ./run-csa-osx.sh <path> [-p] or ./run-csa-osx.sh [-p] <path>"
    exit 1
  else
    if [ $1 == "-p" ] && [ ! -e $2 ]; then 
      echo "[ERROR] Path $2 does not exist. Usage: ./run-csa-osx.sh <path> [-p] or ./run-csa-osx.sh [-p] <path>"
      exit 1
    fi
    if [ $2 == "-p" ] && [ ! -e $1 ]; then 
      echo "[ERROR] Path $1 does not exist. Usage: ./run-csa-osx.sh <path> [-p] or ./run-csa-osx.sh [-p] <path>"
